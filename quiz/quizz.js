...
//    Answer 
    =>  { france : "Paris Saint-Germain",
        spain : "Atlético Madrid"
        bsetTeam: "France national football team",
}
//    ```   
...
//     ```javascript
    ...
    const result = x(3, 3); // Invoke the function with arguments 2 and 3
...
// Answer
// Yes, variables created without any keyword are always global, in the code example provided, the variable a is created without the var, let, or const keyword. This means that a will be created in the global scope rather than the local scope of the x() function.
...
// 11. Which of the following events will you add in the `addEventListener()`method? 
      ☐ click => works in JS
      ☐ onclick => works in HTML and  can only be attached to a single function

// 12. Does the `addEventListener()` method allow you to add many events to the same element?

// Yes, the addEventListener() method allow to add many events to the same element
...
//      ```javascript
         ...
         // every() returns false, because 42 is less than 50
//      ```
...
const number = secondInput.filter(num => num > 0);
...
// objects are passed by reference.
    ...
    changeTshirt(tshirt)
     // the changeTshirt function takes an object myshirt as a parameter. When the function is called with the object tshirt as an argument, a reference to the original tshirt object is passed to the function as myshirt.
//     ```
